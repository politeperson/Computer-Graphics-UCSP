...
void RubikCube3x3::HandleDrawing(glm::mat4& view, glm::mat4& projection, STATE_ANIMATION& some_state)
{
	...
	case STATE_ANIMATION::F:
	{
		...
		if (F()) {
			...
			// el pop debe hacerse una vez se acabe una instrucción
			if (solutionStates.empty()) rubikSolver.F(solution);
			if (!solutionStates.empty()) solutionStates.pop();// aquí
		}
		break;
	}
	case STATE_ANIMATION::f:
	...
	case STATE_ANIMATION::R:
	{
		if (R()) {
			some_state = solutionStates.empty() ? STATE_ANIMATION::NONE : solutionStates.front();
			if (solutionStates.empty()) rubikSolver.R(solution);
			if (!solutionStates.empty()) solutionStates.pop();// aquí
		}
		break;
	}
	...
	case STATE_ANIMATION::U:
	{
		if (U()) {
			some_state = solutionStates.empty() ? STATE_ANIMATION::NONE : solutionStates.front();
			if (solutionStates.empty()) rubikSolver.U(solution);
			if (!solutionStates.empty()) solutionStates.pop();// aquí
		}
		break;
	}
	...
	case STATE_ANIMATION::B:
	{
		if (B()) {
			some_state = solutionStates.empty() ? STATE_ANIMATION::NONE : solutionStates.front();
			if (solutionStates.empty()) rubikSolver.B(solution);
			if (!solutionStates.empty()) solutionStates.pop();// aquí
		}
		break;
	}
	...
	case STATE_ANIMATION::L:
	{
		if (L()) {
			some_state = solutionStates.empty() ? STATE_ANIMATION::NONE : solutionStates.front();
			if (solutionStates.empty()) rubikSolver.L(solution);
			if (!solutionStates.empty()) solutionStates.pop();// aquí
		}
		break;
	}
	...
	case STATE_ANIMATION::D:
	{
		if (D()) {
			some_state = solutionStates.empty() ? STATE_ANIMATION::NONE : solutionStates.front();
			if (solutionStates.empty()) rubikSolver.D(solution);
			if (!solutionStates.empty()) solutionStates.pop();// aquí
		}
		break;
	}
...
//		 EL ERROR SE ENCUENTRA EN EL ÚLTIMO MOVIMIENTO EJECUTADO POR EL SOLVER
//		if (F()) {
//			some_state = STATE_ANIMATION::NONE;
